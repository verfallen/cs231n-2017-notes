...
+ 为了更好的效果，使用集成学习。
+ VGGNet 的最后一个全连接层 F7 能够很好地进行特征表达，在其他任务也有很好的泛化能力。
...
+ 介绍了几种不同的CNN架构，重点研究了4 种主要架构，也是广泛使用的4种。Alex Net 是最早的 CNN 网络之一，非常流行。VGG和 GoogleNet 目前仍然运用很广，ResNet 是后起之秀，目前来看表现最好。
+ 有一种趋势就是追求网络的极致深度
+ 大量研究针对层与层之间的连接设计，或者跳过层连接，哪些需要连接，哪些不需要，根据这些来改进架构设计，改善梯度传递
+ 探讨深度重要还是广度重要，以及残差连接的必要性
