...
在实际应用中们通常采用一种近似方法，称为沿时间的截断反向传播方法。这个方法的思想是
即使输入序列很长很长，甚至趋近于无限。在训练模型时，**前向计算若干步，比如100 。**也就是说前向计算100步，仅仅计算子序列的损失值，然后沿着这个子序列反向传播并计算梯度更新参数。重复上述过程，仍然可以得到网络中的一些隐藏状态。这是从第一批数据中计算得到的，当计算下一批数据时使用这些隐藏状态，所以前向计算过程是相同的。
