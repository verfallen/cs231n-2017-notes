...
f(x,W)=Wx+b
$$
将上述式子中的维度写明，$x$是一个 32x32x3 的向量，展开之后就是一个 3072×1 的向量，输出对应10个类别，因此是一个 10×1 的向量，则 $W$是一个 10×3072 的向量。通常情况下会加上一个偏置项 $b$，$b$是一个 10×1 的常数向量。它不与训练数据交互，仅仅是给一些数据独立的偏好只。如果数据集不平衡，比如猫的数量多于狗的时候
那么与猫对应的偏差值就会比其他要高。